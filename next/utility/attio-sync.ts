...
export async function syncDealToAttio(
  ...
  try {
    ...
    if (!ownerId) {
      console.warn('Could not find Cait Oppermann, deal will be created without owner');
    }
    ...
    const personIds = (await Promise.all(personIdPromises)).filter(id => id !== null) as string[];
    ...
    if (extractedData.collaboratorsNeeded && extractedData.collaboratorsNeeded.length > 0) {
      const collaboratorPromises = extractedData.collaboratorsNeeded.map(collaborator =>
        attioClient.findOrCreateSelectOption('collaborators_needed', collaborator)
      );
    ...
    if (extractedData.dealValue && extractedData.dealValue > 0) {
      payload.values.value = {
        currency: 'USD',
        amount: Math.round(extractedData.dealValue * 100),
      };
    }
